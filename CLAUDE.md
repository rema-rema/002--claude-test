...
### 2025-09-17: トラブルシューティング手法の重大教訓
**事故内容**: VPN/DNS接続問題で「症状から推測」アプローチに固執し、根本原因発見が大幅遅延
**失敗パターン**:
- 症状主導思考: 「pingが通らない」→「netfilter-modeが悪い?」の推測ループ
- 設計書軽視: 基準点を無視し「とりあえず動かそう」の姿勢
- 部分修正: 全体像把握せず一個ずつ問題を直そうとする「木を見て森を見ず」

**突破口**: 「設計書とパラメータシートとの差分を全部箇条書きにして」指示で方針転換
**根本原因**: dnsmasqがTailscaleインターフェース(100.115.216.73:53)で待機していない設定不備
**教訓**: **設計書は「参考」ではなく「絶対的基準点」として最初から扱うべき**
**対策**: トラブル発生時は推測修正より「設計書ベース差分分析」を最優先実行  
...
#### モード管理の優先順位（重要）
1. **機能別作業時**: `spec/kairo/[機能名]/README.md`のモードを絶対優先
2. **全体作業時**: CLAUDE.md全体モードを参照  
3. **Claude指示**: 必ず該当機能のREADME.mdを最初に読み、そのモード制約に従う

#### Kairo機能別モード管理動作フロー
1. kairo機能開発指示を受けた場合 → 即座に`spec/kairo/[機能名]/README.md`を読む
2. そのモード制約を確認 → 制約範囲内でのみ作業実行
3. 全体モード（MAINTENANCE等）は機能別作業時は無視
4. モード違反の指示 → 「[機能名]は現在[モード名]modeのため、[制約内容]」で回答

#### 🏛️ 100回仮想会議レビューシステム（厳守）
**全フェーズ共通の品質保証ルール**:

**対象フェーズ**: REQUIREMENTS、DESIGN、TASK、IMPLEMENTATION、TEST、**CODE**
**実施タイミング**: 各フェーズ完了前（ユーザー確認依頼前）
**必須手順**:
1. **仮想会議実行**: 多様な観点から100回の詳細レビューを実施
   - 参加者構成: PM、アーキテクト、開発者、QA、DevOps、UX、セキュリティ、運用等
   - レビュー観点: 機能性、技術性、運用性、セキュリティ、パフォーマンス、実装可能性
   - 評価基準: 100点満点での評価、改善点の具体的指摘
2. **レビュー結果文書化**: 総合評価、高評価ポイント、改善要求、修正推奨事項
3. **成果物修正**: レビュー結果に基づく必要な修正・追加実施
4. **自動次フェーズ移行**: 品質基準クリア時は即座に次タスクに自動移行

**品質基準**: 総合評価80点以上で自動次タスク移行、80点未満は強制修正
**自動移行ルール**: レビュー完了 → 品質基準クリア → 即座に次タスクに移行（ユーザー確認不要）
**文書化場所**: 各kairo機能のREADME.mdまたは専用レビューファイル

**重要**: レビューが80点以上で完了した場合、ユーザーの指示を待たずに自動的に次のタスクに移行すること。これにより開発効率を最大化する。

#### 📝 CODEフェーズ100回レビュー特別ルール
**実装コード完成時の必須プロセス**:
1. **コードレビュー実行**: 実装完了したコードに対して100回仮想会議実施
2. **レビュー参加者**: アーキテクト、シニア開発者、セキュリティ専門家、QA、DevOps、パフォーマンスチューナー
3. **評価項目**:
   - コード品質・可読性・保守性
   - セキュリティ脆弱性の有無
   - パフォーマンス・効率性
   - エラーハンドリング・例外安全性
   - テスト網羅性・運用性
4. **自動進行**: 80点以上獲得時、次のマイルストーン/タスクに自動移行
5. **改善実装**: 80点未満の場合、指摘事項を即座に修正後再レビュー
...
開発過程で発生した「現在は不要だが将来検討が必要な事項」を管理するための表です。  
優先度・タイミングを明確にして、適切な時期に再検討できるようにします。

| No. | 検討事項 | 理由・背景 | メリット | 検討時期・条件 | 担当 | ステータス |
|-----|----------|------------|----------|---------------|------|------------|
| 1 | **Docker化・コンテナ化** | 現在直接実行で問題なし | 環境統一・並列開発・スケーラビリティ | 並列開発必要時・本格運用時 | 開発チーム | 🔄 棚上げ |
| 2 | 未定 | - | - | - | - | - |
| 3 | 未定 | - | - | - | - | - |
...
- 🔍 **検討中**: 現在調査・検討を進めている
- ✅ **採用決定**: 採用が決定、実装予定  
- ❌ **不採用**: 検討の結果、採用しないことが決定

### 検討事項 No.1 詳細: Docker化・コンテナ化

#### 現在の状況
- **実装方式**: 直接実行（nginx + Next.js + FastAPI）
- **動作状況**: 問題なく動作、検証環境として十分
- **複雑性**: シンプルで管理しやすい

#### Docker化のメリット
- **開発環境統一**: 開発者全員が同じ環境で作業
- **依存関係隔離**: Node.js/Python バージョン競合回避
- **並列開発対応**: 複数ブランチ・セッションの独立実行
- **本格運用**: スケーラビリティ・障害耐性・デプロイ簡素化

#### 検討時のポイント
- **並列開発の必要性**: 複数セッションで同時開発が必要になった場合
- **チーム規模**: 複数の開発者が参加する場合
- **運用規模**: 本格的なサービス運用が必要になった場合
- **学習コスト**: Docker習得・管理にかかる時間対効果

#### 具体的な検討タイミング
1. **並列開発時**: 複数のDiscordセッションで独立した開発環境が必要
2. **チーム開発時**: 複数の開発者が参加し環境統一が必要  
3. **本格運用時**: サービスのスケーラビリティ・可用性が重要になった場合
4. **クラウド移行時**: コンテナベースのクラウドサービス利用時
...
- **起動**: `./bin/vai`
- **状態確認**: `./bin/vai status`
- **セッション監視**: `./bin/vai view`
- **停止**: `./bin/vexit`
- **セッション追加**: `./bin/vai add-session <channel_id>`
- **セッション削除**: `./bin/vai remove-session <session_id>`
- **セッション復旧**: `./bin/vai recover <session_id>`
- **セッション一覧**: `./bin/vai list-sessions`
- **Discord送信**: `dp "メッセージ"` (デフォルトセッション)
- **マルチセッション送信**: `dp 2 "メッセージ"` (セッション2指定)
- **環境診断**: `./bin/vai doctor`
...
### マルチセッション機能
マルチセッション機能により、複数のDiscordチャンネル（A, B, C）で独立したClaude Code開発セッションを並行実行可能。

#### 主要機能
- **独立セッション管理**: 各チャンネルに専用Claude Codeセッション
- **添付ファイル分離**: セッション別ディレクトリ（session_1/, session_2/, session_3/, session_4/）
- **動的セッション管理**: セッション追加・削除・復旧機能
- **セッション間隔離**: 異なるセッション間でのファイル・プロセス競合なし
- **統合管理**: 全セッション状態を統一インターフェースで管理

#### アーキテクチャコンポーネント
- **SessionManager**: セッションライフサイクル・チャンネルマッピング管理
- **AttachmentManager**: セッション別添付ファイル管理
- **TmuxManager**: 複数Claude Codeセッションの分離実行
- **DiscordBot**: メッセージルーティング・セッション指定対応
