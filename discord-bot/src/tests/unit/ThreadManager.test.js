...
import { ThreadManager, ThreadStatus } from '../../components/ThreadManager.js';
...
describe('ThreadManager', () => {
  ...
  describe('正常動作テストケース', () => {
    test('TC-102-001: 承認依頼スレッド正常作成', async () => {
      ...
      expect(result.threadId).toMatch(/^[0-9a-z-]+$/); // Discord ID形式
      expect(result.messageId).toMatch(/^[0-9a-z-]+$/);
    ...
    test('TC-102-003: 24時間自動アーカイブ設定', async () => {
      ...
      // 作成されたスレッドの設定確認
      const createdThread = await threadManager.getThread('thread-456');
    ...
    test('TC-102-006: スレッド状態管理', async () => {
      ...
      // アーカイブ状態に変更
      await threadManager.updateThreadStatus(result.threadId, ThreadStatus.ARCHIVED);
      ...
      // ロック状態に変更
      await threadManager.updateThreadStatus(result.threadId, ThreadStatus.LOCKED);
      thread = await threadManager.getThread(result.threadId);
      expect(thread.status).toBe(ThreadStatus.LOCKED);
  ...
  describe('異常系テストケース', () => {
    test('TC-102-101: Discord API エラー処理', async () => {
      ...
      // Discord API エラーをモック
      mockDiscordClient.channels.threads.create.mockRejectedValueOnce(
    ...
    test('TC-102-103: チャンネル未発見エラー', async () => {
      ...
      const invalidThreadManager = new ThreadManager({
        client: mockDiscordClient,
      ...
      await expect(invalidThreadManager.createApprovalThread({
        approvalRequestId: 'approval-123',
        testName: 'test',
        errorSummary: 'error',
        fixSuggestions: ['fix'],
  ...
  describe('統合テストケース', () => {
    test('TC-102-201: ApprovalRequestManager完全統合', async () => {
      ...
      const integratedThreadManager = new ThreadManager({
        client: mockDiscordClient,
        channelId: 'channel-789',
        autoArchiveDuration: 1440,
      ...
      // 2. スレッド作成
      const threadResult = await integratedThreadManager.createApprovalThread({
      ...
      // 5. スレッド自動アーカイブ確認
      await integratedThreadManager.onApprovalCompleted(updatedRequest, approvalResponse);
      expect(mockDiscordClient.channels.threads.setArchived).toHaveBeenCalledWith(
    ...
    test('TC-102-203: 承認フロー E2E テスト', async () => {
      ...
      const realApprovalManager = new ApprovalRequestManager();
      const e2eThreadManager = new ThreadManager({
        client: mockDiscordClient,
        channelId: 'channel-789',
        autoArchiveDuration: 1440,
        approvalManager: realApprovalManager
      ...
      // Step 2: スレッド作成
      const thread = await e2eThreadManager.createApprovalThread({
      ...
      await e2eThreadManager.sendMessage(
        thread.threadId,
      ...
      await e2eThreadManager.onApprovalCompleted(updatedRequest, response);
    ...
    test('TC-102-205: 長期間スレッド管理', async () => {
      ...
      let thread = await threadManager.getThread(result.threadId);
      expect(thread.status).toBe(ThreadStatus.ACTIVE); // まだアクティブ
      ...
      thread = await threadManager.getThread(result.threadId);
      expect(thread.status).toBe(ThreadStatus.ARCHIVED); // 自動アーカイブ済み
    });
    ...
    test('TC-102-206: エラー復旧・再試行テスト', async () => {
      ...
      const retryThreadManager = new ThreadManager({
        client: mockDiscordClient,
        channelId: 'channel-789',
        autoArchiveDuration: 1440,
        retryAttempts: 3,
      ...
      let attempt = 0;
      const originalCreate = mockDiscordClient.channels.threads.create;
      mockDiscordClient.channels.threads.create = jest.fn().mockImplementation(() => {
        attempt++;
        if (attempt <= 2) {
      ...
      const result = await retryThreadManager.createApprovalThread({
